...
#para_mean<-rowMeans(para)
#para_sd<-apply(para,1,sd)
#pvalue<-2*(1-pnorm(abs(para_mean-c(beta0))/(para_sd/sqrt(run))))
#pvalue
############ one sample t test #####################
library(stats)
t.test(para[1,], alternative = "two.sided", mu = beta0[1,1], conf.level = 0.95)
